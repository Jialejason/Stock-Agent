...
st.set_page_config(page_title="专属私人 AI 投顾 Pro", layout="wide")
st.title("🛡️ 专属私人 AI 量化投顾系统 Pro")
st.caption("⚡ Moomoo 持仓直连 ｜ 🛰️ 盘中量化盯盘 ｜ 🎯 阶梯止盈与移动止损 ｜ 📲 Pushover 实时告警推送")
...
"""
        ...
        position_context = "【用户当前状态】：空仓观望中。请根据严格盈亏比给出最佳开仓条件。"
...
【经典均线与缺口】: EMA5: **${ema5:.2f}** ｜ EMA20生命线: **${ema20:.2f}** ｜ 季线(MA60): {f"${ma60:.2f}" if ma60 else '无'} ｜ 缺口支撑: {f"${gap_support:.2f}" if gap_support else '无'}
...
【核心输出原则】：
1. **直接输出最终中文报告**：严禁输出任何思考过程、英文草稿、自检清单（如 Check list / Tone 说明）或元数据，第一行必须直接从标题或决策灯开始！
2. **彻底说人话，严禁只有黑话！** 遇到专业术语必须紧跟括号大白话说明（例如：VAH(价值区上沿阻力)、POC(最密集主力持仓价)）。
3. 所有价格数字统一紧跟美元符号加粗（如 **$220.50**，**+4.39%**）。
4. 严格按照以下 4 个板块输出：
        ...
        if st.button(f"生成 {selected_pos_sym} 专属投顾报告", type="primary", key="btn_pos_diag"):
            with st.spinner("专属 AI 投顾正在根据你的持仓计算执行指令..."):
                ...
                if d_err:
                    ...
                    col4.metric("建议防守止损", f"${diag_data['dynamic_stop_loss']:.2f}")
...
【均线体系】: EMA5: **${data['ema5']:.2f}** ｜ EMA20: **${data['ema20']:.2f}** ｜ 季线: {data['ma60_str']} ｜ 缺口: {f"${data['gap_support']:.2f}" if data['gap_support'] else '无'}
...
【回答规范】：
1. **直接输出最终中文回答**：禁止输出任何英文思考草稿或自检标签。
2. **直奔主题给结论**：问涨跌空间必做数学计算并给出精确百分比；问买点必给具体价格区间与均线/筹码理由；问盈亏比直接定性（划算/不划算）。
3. **通俗与专业兼备**：如果用到微观指标，必须用大白话解释其对散户交易的实际含义。
4. **数字严格加粗**：所有涉及的价格数字紧跟美元符号加粗（如 **$230.47**，**+4.39%**）。
"""
